...
const loadCredentials = (filePath) => {
...
const ProcessFeed = (ctx, { data = [] }, params = {}) => {
    ...
    const generateDescFromSingleTweet = (item) => {
    ...
    return data.map((item) => {
        const originalItem = item;
        const isThread = item.timeline && item.timeline.length > 0;
        ...
        // If it is thread insert previous tweets before tweet.
        if (isThread) {
            for (const tweet of originalItem.timeline) {
                if (tweet) {
                    description += generateDescFromSingleTweet(tweet);
                    description += `<br><br>`;
                }
            }
        }
