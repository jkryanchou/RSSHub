...
logger.info(`Init account: ${randomAccount}`);
...
const getCookieValue = (cookie, fieldName) => {
...
async function twitterRequest(url, params, method) {
    ...
    try {
        ...
        const rateRemaining = parseInt(response.headers['x-rate-limit-remaining']);

        if (rateRemaining <= config.twitter.rateRemainingThreshold) {
