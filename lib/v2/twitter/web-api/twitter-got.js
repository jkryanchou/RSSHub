...
//     const twids = Object.keys(credentials);

//     // NOTE: requestRemaining === -1 means initial state
//     const available = twids.find(
//         (twid) => twid !== current && (
//             credentials[twid].requestRemaining > 10 || credentials[twid].requestRemaining === -1
//         )
//     );

//     if (available) {
//         headers.authorization = available.authorization;
//         headers["x-csrf-token"] = available.csrfToken;
//         headers.cookie = available.cookie;
//     } else {
//         throw Error('No request remaining avaliable credential');
//     }
// }
...
// function updateCredential(response) {
//     const twid = getCookieValue(response.request.options.headers.cookie, 'twid').replace('u%3D', '');
//     credentials[twid].requestRemaining = parseInt(response.headers['x-rate-limit-remaining'], 10);
//     credentials[twid].resetTime = parseInt(response.headers['x-rate-limit-reset'], 10);
// }
