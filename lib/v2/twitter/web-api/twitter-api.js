...
function gatherLegacyFromDataUpgrade(entries, filterNested = undefined) {
    const tweets = [];
    ...
    entries.forEach((entry) => {
        ...
        if (entryId) {
            ...
            if (filterNested && filterNested.some((f) => entryId.startsWith(f))) {
                ...
                const lastTweet = entry.content.items[entry.content.items.length - 1];
        ...
        if (entry.entryId) {
            ...
            if (entry.entryId.startsWith('profile-conversation-')) {
                ...
                for (const t of entry.timeline) {
                    // Exclude last tweet
                    if (t === entry.timeline[entry.timeline.length - 1]) {
                        continue;
                    }
    ...
    for (const t of [tweet, retweet]) {
        ...
        if (quote && quote.legacy) {
            ...
            if (quote.core) {
                ...
                t.legacy.quoted_status.user = quote.tweet.core.user_results.result.legacy;
            }
        }
    }
...
const getUserTweetsAndRepliesByID = async (id, params = {}) => gatherLegacyFromDataUpgrade(await timelineTweetsAndReplies(id, params), ['profile-conversation-'], id);
